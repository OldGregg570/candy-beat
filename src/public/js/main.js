...
angular.module('CandyBeatApp').directive('candymain', function () {
    return {restrict: 'E', scope: {}, controller: 'mainCtrl', templateUrl: './src/public/html/main.html'}
});

angular.module('CandyBeatApp').controller('mainCtrl',

    function ($scope, Namer, Track, Cell, $interval, synthService, music, $modal, $timeout, randomService, logger, randomizerService, midi) {
        ...
        $scope.volumeSlider = {
            floor: 0, ceil: 100, onChange: function () {
                $scope.onVolumeChange();
            }
        };
        $scope.tempoSlider = {
            floor: 1, ceil: 100, onEnd: function () {
                $scope.onTempoChange();
            }
        };
        ...
        $scope.tick = function tick() {
            ...
            var soloTracks = $scope.tracks.filter(function (e) {
                return e.solo;
            });
            var tracks = (soloTracks.length > 0 ? soloTracks : $scope.tracks).filter(function (e) {
                return !e.mute;
            });
                ...
                for (var c in track.columns[$scope.activeColumn]) {
                    ...
                    if ($scope.activeColumn % r === 0 && cell.active) {
                        ...
                        if (track.midiOutput.internal) {
                            ...
                            midi.playNote($scope.scale.notes[c] + 60, track.midiOutput.id, track.midiChannel)
                        }
        ...
        $scope.randomizeTracks = function () {
            var tracks = $scope.tracks.filter(function (t) {
        ...
        $scope.openTrackSettings = function (track) {
            var modalInstanccee = $modal.open({
                animation: true,
                templateUrl: './src/public/html/track-settings.html',
        ...
        $scope.openPreferences = function () {
            var modalInstance = $modal.open({
                animation: true,
                templateUrl: './src/public/html/preferences.html',
        ...
        $scope.openAnalyser = function () {
            var modalInstance = $modal.open({
                animation: true,
                templateUrl: './src/public/html/analyser.html',
...
angular.module('CandyBeatApp').directive('toolbar', function () {
    return {scope: false, templateUrl: './src/public/html/templates/toolbar.html'}
});

angular.module('CandyBeatApp').directive('mixer', function () {
    return {scope: false, templateUrl: './src/public/html/templates/mixer.html'}
});

angular.module('CandyBeatApp').directive('mainmenu', function () {
    return {scope: false, templateUrl: './src/public/html/templates/main-menu.html'}
});
